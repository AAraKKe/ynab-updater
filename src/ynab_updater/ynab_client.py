...
class NetWorthResult:
    ...
    def update(self, other: NetWorthResult):
    ...
    @staticmethod
    def from_accounts(accounts: list[Account]) -> NetWorthResult:
        cash = []
        savings = []
        debt = []
        assets = []
        ...
        for account in accounts:
            ...
            match _type:
                case NetworthType.CASH:
                    cash.append(balance)
                case NetworthType.SAVINGS:
                    savings.append(balance)
                case NetworthType.DEBT:
                    debt.append(balance)
                case NetworthType.ASSETS:
...
class YnabHandler:
    ...
    def net_worth(self, budget_id: str) -> NetWorthResult:
        accounts = self.get_accounts(budget_id)
        return NetWorthResult.from_accounts(accounts)


__client: YnabHandler | None = None
...
def set(api_key: str | SecretStr, logger: Logger) -> YnabHandler:
    ...
    __client = YnabHandler(api_key=api_key, logger=logger)
    return __client
...
def get() -> YnabHandler:
    ...
    if __client is None:
        ...
        if (api_key := config.ynab_api_key) is not None:
            __client = YnabHandler(api_key=api_key, logger=Logger(None))
            return __client
